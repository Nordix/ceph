...
bool DaemonServer::ms_handle_reset(Connection *con)
{
  ...
  // Connections are inserted into daemon_connections for *any* non-client
  // daemon (mon, mds, osd, ...) in handle_open(), so the ConnectionRef must
  // be released here for all peer types, not just OSDs.  Otherwise every
  // ungracefully reset non-OSD daemon connection leaks its AsyncConnection.
